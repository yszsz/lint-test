...
import { toFormData } from '../pureUtils/object';
...
function responseDataStatus(validateStateError) {
  const defaultErrorValidate = _responseData => {
    return (
      _responseData.status && _responseData.status.toUpperCase() == 'ERROR'
    );
  };
...
function responseAuthorityValidator(_options) {
  ...
  return {
    responseError(_responseError, _request) {
      ...
      const {
        authorityValidator,
        authorityFailureCodes,
        authorityFailureHash,
        afterAuthorityFailure,
      } = _request.options;
      const authorityOptions =
        applyFn(_options, _request, _responseError) || defaultOptions;

      if (authorityValidator) {
        return Promise.reject(
          authorityValidator(_responseError, _request) || _responseError
        );
      }
      ...
      if (errorCode && codes.includes(`${errorCode}`)) {
        if (!hasErrorMessage) {
          const errorMessage =
            _responseError.errorMsg || DEFAULT_EXPIRE_MESSAGE;
          message.warning(errorMessage, 2, () => (hasErrorMessage = false)); // eslint-disable-line
...
function responseErrorHandler() {
  ...
  function defaultResponseErrorHandler(_responseError, _request) {
    if (!_request.options.ignoreErrorModal && !hasErrorModal) {
      const title =
        transformErrorMsg(_responseError.message) ||
        _responseError.errorMsg ||
        _responseError.error_message ||
        DEFAULT_RES_ERROR;
  ...
  return {
    responseError(_responseError, _request) {
      ...
      let _responseErrorResult; // eslint-disable-line
