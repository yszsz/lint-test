...
export function getEffect(effectName, curNamespace) {
  const targetNamespace = effectName.includes('/')
    ? effectName.split('/')[0]
    : curNamespace;
  const targetEffectName = effectName.includes('/')
    ? effectName
    : `${curNamespace}/${effectName}`;
  // eslint-disable-next-line
  const targetModel = window.$$dvaApp._models.find(
...
export function getModel(targetNamespace) {
  ...
  // eslint-disable-next-line
  return window.$$dvaApp._models.find(
    ({ namespace }) => namespace == targetNamespace
  );
}
...
export function model(...keys) {
  return function tar(target) {
    const mapStateToProps = state =>
      keys.reduce((prev, next) => ({ ...prev, ...state[next] }), {});
